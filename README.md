...
- 展览页 `index.html`：一行 4 张照片墙，默认展示最新 12 张（「更多」按钮展开全部），点击看详情；底部开放留言板；页脚显示最后更新日期；回到顶部按钮
- 登录页 `login.html`：账号 `wenlinshu`（密码 `yyyuzengyang`）/ `yuzengyang`（密码 `wwwenlinshu`），封面用真实照片
- 管理页 `admin.html`：上传（自动压缩 + 进度条）、批量统一地点、编辑（含可见性设置）、删除、留言管理、存储设置
...
├── photos.json       照片元数据（地点、日期、介绍、伙伴标签、可见性）
├── messages.json     留言板数据
├── photos/           照片文件（上传后自动写入）
├── assets/
